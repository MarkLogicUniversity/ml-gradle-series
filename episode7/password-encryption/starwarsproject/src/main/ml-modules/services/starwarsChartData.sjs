...
function get(context, params) {
  ...
  const weaponElement = fn.QName("","weapon");
  const allianceElement = fn.QName("","alliance");
  const bioElement = fn.QName("","bio");
  ...
  const weaponElementRef = cts.elementReference(weaponElement);
  const allianceElementRef = cts.elementReference(allianceElement);
  const bioElementRef = cts.elementReference(bioElement);
  ...
  for (let name of cts.values(charNameElementRef, null, sortOrder, viewQuery)) {
    ...
    const charWeapon = cts.values(weaponElementRef, null, null, query);
    const charAlliance = cts.values(allianceElementRef, null, null, query);
    const charBio = cts.values(bioElementRef, null, null, query);
